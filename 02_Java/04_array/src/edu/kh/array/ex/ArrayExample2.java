...
//	    하나의 배열(또는 객체)를 참조하여 상태를 만드는 복사 방법
	...
	public void deepCopy() {
		...
		// 2. System.arraycopy(원본 배열, 원본 복사 시작 인덱스, 복사당할 배열, 복사당할 배열
//		  삽입 시작 인덱스, 복사길이 (보통 원본 배열의 길이) )
		
		// 3. Arrays.copyOf(원본배열, 복사할길이);
	...
	public void createLottoNumber() {
		...
		for(int i = 0; i < lotto.length; i++) {
			...
			int random = (int)Math.floor(Math.random() * 45 + 1);
			...
			for(int x = 0; x < i; x++) {
				...
				if(random == lotto[x]) {
					...
					// i값을 인위적으로 1 감소시켜서 총 7회 반복되는 모양을 만든것.
					break;
				}
			}
			...
			//Array.sort(배열명) : 배열 내 값들이 오름차순으로 정렬됨
